...
import { LayoutDashboard, FileText, PackageCheck, Package, AlertCircle, Truck, LogOut } from "lucide-react";
...
const nav = [
  ...
  { name: "Dispense", href: "/pharmacy/dispense", icon: PackageCheck },
  { name: "Inventory", href: "/pharmacy/inventory", icon: Package },
  { name: "Expiry Alerts", href: "/pharmacy/expiry", icon: AlertCircle },
    ...
    <div className="flex h-screen w-full overflow-hidden bg-background">
      <aside className="flex w-60 flex-shrink-0 flex-col border-r border-border bg-[#8B1A2F]">
        <div className="flex items-center gap-2 border-b border-white/15 px-3 py-4">
          ...
          <p className="text-[10px] text-white/60 font-medium">Pharmacy</p>
        </div>
        <nav className="flex-1 overflow-y-auto px-2 py-4 space-y-0.5">
          {nav.map((item) => {
            const active = location === item.href || (item.href !== "/pharmacy" && location.startsWith(item.href));
            return (
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="flex h-16 items-center justify-between border-b border-border px-6 bg-background">
          <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-green-100 text-green-700">Pharmacy Portal</span>
          <div className="flex items-center gap-3">
            <NotificationBell role="pharmacy" />
            <ProfileDropdown />
          </div>
        </header>
